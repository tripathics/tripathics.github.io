...
export default function Loading({ isLoading = true }: { isLoading?: boolean }) {
  ...
  const initialLoading = useRef(isLoading).current;
  ...
  return (
    counter > -2 && (
      <>
        <noscript>
          <style>{`.loading-component2 { display: none; }`}</style>
        </noscript>
